...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        // Execute the Python flexible_scheduling module
        $command = 'python backend/ml_models/flexible_scheduling.py /tmp/teachers_input.json /tmp/classes_input.json /tmp/constraints_input.json /tmp/preferences_input.json';
        exec($command, $output, $return_var);

        if ($return_var !== 0) {
            error_log("Flexible scheduling script failed with return code $return_var");
            http_response_code(500);
            echo json_encode(['error' => 'Failed to execute flexible scheduling']);
            exit;
        }
